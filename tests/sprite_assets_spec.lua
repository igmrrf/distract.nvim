...
local function delta(a, b)
  ...
  for y = 1, #a do
    for x = 1, #a[y] do
      ...
      if pa and pb then
        sum = sum + (math.abs(pa[1] - pb[1]) + math.abs(pa[2] - pb[2])
          + math.abs(pa[3] - pb[3])) / (3 * 255)
...
local function differs(a, b)
  for y = 1, #a do
    for x = 1, #a[y] do
      local pa, pb = a[y][x], b[y][x]
      if (pa and not pb) or (pb and not pa) then return true end
...
describe("generated sprite geometry", function()
    ...
    for name, spec in pairs(ASSETS) do
      ...
      for i, matrix in ipairs(frames) do
        ...
        for y, row in ipairs(matrix) do
          assert(#row == spec.w, string.format(
            "%s frame %d row %d has %d cells, expected %d", name, i, y, #row, spec.w))
        end
      end
    end
  end)
  ...
  it("caches generated frames instead of redrawing on every call", function()
    for name, _ in pairs(ASSETS) do
      assert(sprites.get_pixel_frames(name) == sprites.get_pixel_frames(name),
  ...
  it("draws something in every frame", function()
    for name, _ in pairs(ASSETS) do
      for i, matrix in ipairs(sprites.get_pixel_frames(name)) do
        assert(filled_cells(matrix) > 8, string.format(
          "%s frame %d is effectively empty (%d pixels)", name, i, filled_cells(matrix)))
      end
    end
  end)
end)
  ...
  it("resolves every manifest state to frames that exist", function()
    for name, spec in pairs(ASSETS) do
      ...
      for state, def in pairs(spec.manifest.states) do
        ...
        assert(frames and #frames > 0, string.format("%s state '%s' declares no frames", name, state))
        for _, idx in ipairs(frames) do
          assert(idx >= 0 and idx < count, string.format(
            "%s state '%s' references frame %d, outside 0..%d", name, state, idx, count - 1))
        end
      end
    end
  end)
    ...
    for name, spec in pairs(ASSETS) do
      ...
      for a, key_a in pairs(first_by_state) do
        for b, key_b in pairs(first_by_state) do
          if a < b then
            assert(key_a ~= key_b, string.format(
              "%s states '%s' and '%s' open on identical art", name, a, b))
          end
        end
      end
    end
  ...
  it("animates every multi frame state instead of holding one pose", function()
    for name, spec in pairs(ASSETS) do
      ...
      for state, def in pairs(spec.manifest.states) do
        ...
        if #list > 1 then
          local keys = {}
          for _, idx in ipairs(list) do keys[frame_key(frames[idx + 1])] = true end
          assert(vim.tbl_count(keys) > 1, string.format(
            "%s state '%s' lists %d frames that are all identical", name, state, #list))
        end
      end
    end
  end)
      ...
      for state, def in pairs(spec.manifest.states) do
        ...
        for i = 2, #list do
          if list[i] ~= list[i - 1] then
            assert(differs(frames[list[i - 1] + 1], frames[list[i] + 1]), string.format(
              "%s state '%s': frames %d and %d are identical",
              name, state, list[i - 1], list[i]))
          end
        end
      end
    end
