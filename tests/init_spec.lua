...
describe("distract.init backend availability", function()
  ...
  it("only advertises backends that are actually implemented", function()
    ...
    for _, name in ipairs(distract.get_available_backends()) do
      ...
      assert(
        implemented,
        string.format("backend '%s' is advertised but has no renderer implementation", name)
      )
    end
  end)
  ...
  it("does not advertise the unimplemented kitty graphics backend", function()
    distract.setup()
    assert.is_false(
      vim.tbl_contains(distract.get_available_backends(), "kitty"),
  ...
  it("tells the user when a requested backend was substituted", function()
    ...
    for _, msg in ipairs(messages) do
      if tostring(msg):lower():match("kitty") then
        mentioned = true
      end
    end
    assert.is_true(mentioned, "substituting an unavailable backend must be reported, not silent")
    distract.setup({ backend = "halfblock" })
  end)
end)
...
describe("distract.init asset & action query methods", function()
  ...
  it("stop resets warmup background jobs and clears running state", function()
    local warmup = require("distract.warmup")
    distract.setup({ backend = "halfblock" })
    distract.start()
    assert.is_true(distract.is_running())

    warmup.request("test_warmup", function()
      coroutine.yield()
    end)
    assert.is_true(warmup.is_pending("test_warmup"))

    distract.stop()
    assert.is_false(distract.is_running())
    assert.is_false(warmup.is_pending("test_warmup"))
